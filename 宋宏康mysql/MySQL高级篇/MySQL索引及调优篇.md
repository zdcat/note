...
我们把具有这两种特性的B+树称为聚簇索引，所有完整的用户记录都存放在这个`聚簇索引`的叶子节点处。这种聚簇索引并不需要我们在MySQL语句中显式的使用INDEX 语句去创建， `InnDB` 存储引擎会 `自动` 的为我们创建聚簇索引。（从一开始就会默认创建一个B+树）

**优点：**

* `数据访问更快` ，因为聚簇索引将索引和数据保存在同一个B+树中，因此从聚簇索引中获取数据比非聚簇索引更快 
* 聚簇索引对于主键的 `排序查找` 和 `范围查找` 速度非常快 ，因为页面之间是双向链表
* 按照聚簇索引排列顺序，查询显示一定范围数据的时候，由于数据都是紧密相连，数据库不用从多 个数据块中提取数据，所以 `节省了大量的io操作` 。

**缺点：**

* `插入速度严重依赖于插入顺序` ，按照主键的顺序插入是最快的方式，否则将会出现页分裂，严重影响性能。因此，对于InnoDB表，我们一般都会定义一个`自增的ID列为主键`，随便插入肯定导致b+树的平衡操作不断发生，所以插入速度依赖于插入顺序
* `更新主键的代价很高` ，因为将会导致被更新的行移动。因此，对于InnoDB表，我们一般定义**主键为不可更新**，同样更新主键也会导致平衡操作发生，肯定也会耗时
* `二级索引访问需要两次索引查找` ，第一次找到主键值，第二次根据主键值找到行数据

![image-20230303180929028](img/image-20230303180929028.png)

主键唯一、非空且唯一，尽量选有序的字段，而不是无序的字符串什么的，只要有序就可以保证b+树的创建



#### 2. 二级索引（辅助索引、非聚簇索引）

<font color='red'>聚簇索引只有一个，二级索引可以有很多个</font>
...
<font color='red'>在二级索引对应的这棵B+树种，在叶子结点里面把二级索引和聚簇索引关联起来（而且叶子节点存储的行记录里没有其他多余的列），然后通过回表再去聚簇索引对应的B+树再查一遍，剩下的基本一样  </font>
...
如果把完整的用户记录放到叶子结点是可以不用回表。但是`太占地方`了，相当于每建立一课B+树都需要把所有的用户记录再都拷贝一遍，这就有点太浪费存储空间了。<font color='red'>假如一个表里有1kw条数据，5个非聚簇索引，那么假如非聚簇的叶子结点都要保存整行信息的话，那需要的空间就太多了，而且修改的时候花费的时间会更多，所以二级索引的查询必须要回表到聚簇索引再查一遍，所以说非聚簇索引的叶子结点只需要存储非聚簇索引对应的那一行的主键就好了</font>
...
3. 使用聚簇索引的时候，数据的`查询效率高`，但如果对数据进行插入，删除，更新等操作，效率会比非聚簇索引低。<font color='red'>查询不用说，需要回表，肯定聚簇索引快，修改聚簇索引的时候，其他的非聚簇索引可能都要改，而修改非聚簇索引的时候，只需要更改对应的B+树和聚簇索引对应的B+树里面的那一行的数据即可，修改聚簇索引可能会发生很大改变！</font>

#### 3.联合索引

<font color='red'>联合索引本质其实就是二级索引，只不过是两个或者多个一起作为非聚簇索引</font>
...
<font color='red'>头脑模拟一下多叉平衡树的插入过程</font>
...
<font color='red'>即二级索引的页目录实质上</font>
...
<font color='red'>多叉树最少也得是个二叉树才有意义，这样才能减少树的高度，即从N变为log N（2为底）</font>
...
B 树的英文是 Balance Tree，也就是 `多路平衡查找树`。简写为 B-Tree。它的高度远小于平衡二叉树的高度。<font color='red'>B树本质就是AVL的多叉版本，和B+树的区别很大</font>
...
<font color='red'>注意别忘了B树是AVL的多叉版本，也就是说有时候查询的时候不一定要像B+树一样查到叶子结点，遵循AVL的查找逻辑，比如查询主键为8，只需要查到第二层就返回了。</font>

<font color='red'>而且要注意，对于每一个节点，不管其在哪一层，都会像B+树的叶子节点一样，存储的是主键及其对应的那一行的数据，仔细看上面的图，每个节点里面都存在着主键以及主键这一行的数据，这就导致B树的节点里放不下太多的东西（对比B+树的目录页的话肯定要少不少，因为B+树目录页存放的是主键以及下一个页的地址而不是一整行数据），就会导致B树的数的高度一般情况下会比B+树要高，自然效率比不上B+树</font>
...
<font color='red'>范围查询B+树的叶子结点之间是链表，进行范围查询很方便，而B树的设计是每个节点都内部都存放行，所以要想进行范围查询，只能进行中序遍历，那就涉及到了不同页之间的切换，范围查找的速度肯定是不如B+树快的</font>

<font color='red'>记住B树是AVL的多叉版本，以及B树里面每个节点都存放着主键以及行数据就好，因此导致每个节点里放的</font>
...
<font color='red'>8B+8B中，第一个8B指的是主键的大小，第二个8B指的是这个主键对应的目录页或者数据页的大小，这俩在一起正好16B</font>
...
<font color='red'>哈希索引在构造联合索引的时候是把这N个字段联合在一起去创建索引的，假如仅仅只用到其中的1个或者M（M<N）个，那么就会导致之前构建的联合索引用不到了</font>
...
<font color='red'>候选键就是最简单能决定一行的列的集合，有时候一列就可以决定一行，有时候两列才能决定一行，候选键可以有多个</font>
...
此时，候选键有两个，球员编号和身份证号，这两个都是候选键，所以主键可以选择这两个任意一个

再比如

![image-20230405130027320](img/image-20230405130027320.png)

一个仓库名和一个管理员是一对一的，即我们知道仓库名就知道管理员，知道管理员就知道仓库名，此时可以发现，（仓库名，物品名）可以决定一行数据，（管理员，物品名）也可以决定一行数据，所以此时候选键有两个，（仓库名，物品名）和（管理员，物品名），所以主键可以选择这两个
...
<font color='red'>第二范式记住这一句话就好了，所有非主键字段必须要完全依赖于主键</font>
...
<font color='red'>比如姓名是不依赖比赛编号的，姓名只球员编号，因为此时是联合主键(球员编号, 比赛编号)，所以姓名此时依赖部分主键，不是完全依赖主键，所以不满足第二范式</font>
...
<font color='red'>二范式要求非主键字段不能依附于部分主键，也就是说非主键字段要完全依赖于主键</font>

<font color='red'>三范式要求非主键字段之间不能相互依赖</font>
...
<font color='red'>第一范式：每个列都要保证原子性</font>

<font color='red'>第二范式：保证了第一范式的基础上，所有非主键列必须依赖于整个主键，而不能依赖主键的一部分（很多时候都是针对于联合主键说明的）</font>

<font color='red'>第三范式：保证了第一、第二范式的基础上，所有非主键列之间不能有依附关系，即非主键之间要相互独立，与主键的关系必须是直接相关而不是间接相关</font>
...
<font color='red'>第三范式消除了非主属性对于非主属性的依赖，BCNF在3NF的基础上消除了主属性对候选键的部分依赖</font>
...
<font color='red'>这样就不存在主属性对于候选键的部分依赖或传递依赖，上面数据表的设计就符合 BCNF。</font>
...
<font color='red'>这里的意思是不在数据库里去加外键，而是在业务层里加外键，也就是说外键加的越多，表与表本来之间的关系会更加紧密</font>
...
<font color='red'>分析表用来查看字段区分度大小，从而判断是否适合加索引</font>
