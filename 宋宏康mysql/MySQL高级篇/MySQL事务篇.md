...
* redo log: 是存储引擎层 (innodb) 生成的日志，记录的是`"物理级别"`上的页修改操作，比如页号xxx，偏移量yyy写入了'zzz'数据。主要为了保证数据的可靠性。<font color='red'>注意redo log是物理级别的日志，即哪个页多少偏移量写入几，这种物理级别</font>
* undo log: 是存储引擎层 (innodb) 生成的日志，记录的是 `逻辑操作` 日志，比如对某一行数据进行了INSERT语句操作，那么undo log就记录一条与之相反的DELETE操作。主要用于 `事务的回滚` (undo log 记录的是每个修改操作的 `逆操作`) 和 `一致性非锁定读` (undo log 回滚行记录到某种特定的版本——MVCC，即多版本并发控制)。<font color='red'>undo log是逻辑日志，记录的是事务的语句的相反的逻辑的日志，你事务执行一个insert操作，undo log就记录一个delete操作，这种逻辑日志</font>
...
<font color='red'>下面的流程很重要</font>

```
第1步：先将原始数据从磁盘中读入内存中来，修改数据的内存拷贝
第2步：生成一条重做日志并写入redo log buffer，记录的是数据被修改后的值
	（这里一定要注意，内存的数据拷贝修改完了，然后redo log buffer就有记录了，注意redo log buffer记录的是物理级别的数据页的改动）
第3步：当事务commit时，将redo log buffer中的内容刷新到 redo log file，对 redo log file采用追加写的方式
	（这里可以理解成commit和redo log buffer的内容刷新到redo log file里是同步发生的，即commit成功就会刷新，commit不成功就不会刷新，也好理解，		 commit成功了，那么redo的日志文件一定是要保存的，以防到时候data buffer里的数据没有刷新到data里，如果commit不成功，那么就不会从redo log 		 buffer刷新到redo log file里，没有commit成功的数据自然不需要持久化）
...
* `设置为0` ：表示每次事务提交时不进行刷盘操作。（系统默认master thread每隔1s进行一次重做日 志的同步） 第1步：先将原始数据从磁盘中读入内存中来，修改数据的内存拷贝 第2步：生成一条重做日志并写入redo log buffer，记录的是数据被修改后的值 第3步：当事务commit时，将redo log buffer中的内容刷新到 redo log file，对 redo log file采用追加 写的方式 第4步：定期将内存中修改的数据刷新到磁盘中 
  ...
  <font color='red'>0即事务提交不刷盘，由主线程每隔1s去做redo log buffer到redo log file的刷盘，存在redo log buffer直接满了的情况，此时直接写入redo log file</font>

* `设置为1` ：表示每次事务提交时都将进行同步，刷盘操作（ 默认值 ） 

  <font color='red'>1即每次事务提交，就会进行redo log buffer到redo log file的刷盘</font>

* `设置为2` ：表示每次事务提交时都只把 redo log buffer 内容写入 page cache，不进行同步。由os自 己决定什么时候同步到磁盘文件。

  <font color='red'>2即每次事务提交都把redo log buffer写入page cache，文件系统缓存，而不写入redo log file，什么时候从page cache到redo log file由OS决定</font>

<font color='cornflowerblue'>可以发现，只有当事务提交了，才会进行把redo log buffer里面的数据往外写出的操作，这是合情合理的，只有COMMIT了，redo log buffer里面的数据才有用，如果没有COMMIT成功，那么redo log buffer里面的数据就没用，事务都失败了还需要什么持久化呢？</font>
...
<font color='red'>注意这个刷盘策略指的是事务提交时，redo log buffer刷盘到redo log file的策略，分三种情况</font>
...
<font color='red'>所以说参数设置为1是最安全的情况</font>

1. <font color='red'>事务提交成功，redo log buffer刷盘到redo log file里，持久化就做好了</font>
2. <font color='red'>事务提交失败，redo log buffer不会刷盘到redo log file里，但是本身失败的事务就不需要持久化</font>
...
<font color='red'>事务提交成功之后，redo log buffer里面的内容写到文件系统缓存，然后再从文件系统缓存刷盘进redo log file，安全性全看OS了，如果OS宕了，那就会丢失数据，不过OS一般不会宕</font>
...
<font color='red'>参数为0，就是事务提交了，由主线程来负责redo log buffer到redo log file的刷盘，如果数据库宕了，那么就会有数据丢失</font>
...
<font color='red'>这也是为什么undo log是逻辑级别的日志，假如你是物理级别的日志，那你到时候一恢复，数据页变成了原来的样子，那假如在你恢复数据页的原样的之前有其他事务对这个页做了修改怎么办，所以必须要逻辑恢复</font>

* **作用2：MVCC**

undo的另一个作用是MVCC，即在InnoDB存储引擎中MVCC的实现是通过undo来完成。当用户读取一行记录时，若该记录以及被其他事务占用，当前事务可以通过undo读取之前的行版本信息，以此实现非锁定读取。

<font color='red'>可以通过MVCC这个机制读取到这个行的版本信息，从而实现这个行的非锁定读</font>
...
<font color='red'>这里发现一个很有意思的点，先记录undo log，然后执行sql改内存数据，然后记录redo log，然后commit的时候根据刷盘策略来实现redo log buffer到redo log file的刷盘</font>
...
<font color='red'>以上可以发现不更新主键的时候就是单纯的把旧的数据写入到undo log里面</font>
...
<font color='red'>对于更新主键的操作，就是在把旧行的删除标记打开，然后在旧行后面插入一个主键变更的新行（这个逻辑和插入的undo逻辑是一样的），假如要不回滚，因为旧行的删除标记是1，所以旧行会被删除，假如回滚，新行的删除逻辑和插入的删除逻辑是一样的</font>
...
<font color='red'>完全可以通过记录的版本链完成回滚，因为要回滚的数据都记录好了</font>
...
* 针对于insert undo log
  ...
  <font color='red'>对，因为是新数据的插入，别的事务根本不知道你新插入了数据，所以这部分新的数据的undo log日志直接删除了就好了</font>

* 针对于update undo log

  该undo log可能需要提供MVCC机制，因此不能在事务提交时就进行删除。提交时放入undo log链表，等待purge线程进行最后的删除。
  
  <font color='red'>旧行的更新涉及到了MVCC，所以不是立即删除</font>
...
<font color='red'>用索引定位更少的行，因为遍历到的行都会被上锁</font>
...
* 如果被访问版本的trx_id属性值与ReadView中的 creator_trx_id 值相同，意味着当前事务在访问它自己修改过的记录，所以该版本可以被当前事务访问。 
  ...
  <font color='red'>假如最近修改这条记录的事务A和创建ReadView的事务B的id是一个，那就说明这个事务A一定是先做了修改，再去创建的ReadView，那么这条记录在这个ReadView中一定是可见的，这个判断规则很容易理解</font>

* 如果被访问版本的trx_id属性值小于ReadView中的 up_limit_id 值，表明生成该版本的事务在当前事务生成ReadView前已经提交，所以该版本可以被当前事务访问。 

  <font color='red'>假如最近修改这条记录的事务A小于此时ReadView中的最小的活动事务，那就说明事务A此时一定已经不活跃了（如果活跃那么一定就会被记录在活跃事务里面了），那么事务A做的修改一定已经提交了，那么已经提交的修改必然是可见的，毕竟是读已提交</font>

* 如果被访问版本的trx_id属性值大于或等于ReadView中的 low_limit_id 值，表明生成该版本的事务在当前事务生成ReadView后才开启，所以该版本不可以被当前事务访问。 

  <font color='red'>这里我是这么理解的：创建ReadView的事务B此时会把活跃的事务id都记录起来，假如你当前判断的这个行记录的对应的事务A的id比要分配的事务id都要大，说明一定有新的事务在事务B生成了ReadView后做了新的更改，那么这个新的更改应该是不属于当前这个ReadView快照里的。自然这条数据的这个版本对于此时的ReadView是不可见的，要理解快照的的含义，不属于快照的自然不可见（毕竟事务B只是做快照读，不上锁，其他事务是随意更新然后COMMIT的）</font>

* 如果被访问版本的trx_id属性值在ReadView的 up_limit_id 和 low_limit_id 之间，那就需要判断一下trx_id属性值是不是在 trx_ids 列表中。
  * 如果在，说明创建ReadView时生成该版本的事务还是活跃的，该版本不可以被访问。 
  
    <font color='red'>仍活跃即未提交，未提交即不可见</font>
...
<font color='red'>根据ReadView生成的策略，来实现不同隔离级别的效果</font>

<font color='red'>ReadView只需要生成对应快照时期的数据（最小事务id、最大事务id、活跃事务id组），配合版本链和过滤规则就可以找到建造ReadView的这个事务进行快照读要查的那条数据了 </font>
