...
* `READ UNCOMMITTED` ：读未提交，在该隔离级别，所有事务都可以看到其他未提交事务的执行结 果<font color='red'>即要进行脏写的时候A会看到B的写入结果，所以A会等待B执行完才会重新进行更新数据，所以就避免了藏独</font>。不能避免脏读、不可重复读、幻读。 
