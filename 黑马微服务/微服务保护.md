...
<font color='red'>业务1分配10个线程，业务2页分配10个线程，如果服务C挂了只会影响tomcat分配给业务2的10个线程，分配给服务B的10个线程不会被影响，就很像船舱的感觉，因为业务1还有10个线程，所以不会影响服务B的执行，所以阻断了雪崩</font>
...
- <font color='red'>微服务之间相互调用，因为调用链中的一个服务故障，引起整个链路都无法访问的情况。</font>



可以认为：

<font color='red'>**限流**是对服务的保护，避免因瞬间高并发流量而导致服务故障，进而避免雪崩。是一种**预防**措施。</font>

<font color='red'>**超时处理、线程隔离、降级熔断**是在部分服务故障时，将故障控制在一定范围，避免雪崩。是一种**补救**措施。</font>

超时处理其实不好用，假如你一个服务最长等待1s，1s过后就算处理完了（对应的超时逻辑），那么假如1s有两个请求来，那么入不敷出，还会雪崩

线程隔离会导致资源浪费，假如你业务2调用的服务C一直是挂的，那么tomcat给业务2分配的10个线程就一直闲置的
...
|                | **Sentinel**                                       | **Hystrix**                   |
| -------------- | -------------------------------------------------- | ----------------------------- |
| 隔离策略       | 信号量隔离（限制每个业务的tomcat提供的线程的数量） | 线程池隔离/信号量隔离         |
...
<font color='red'>sentinal实现了限流、熔断、线程隔离（仓壁）</font>
...
<font color='red'>sentinel本质是个jar包，我们只需要启动jar包就好了</font>
...
<font color='red'>sentinal貌似是个springboot项目</font>
...
我们在**order-service中整合sentinel**，并连接sentinel的控制台，步骤如下：
...
例如，我们刚才访问的order-service中的OrderController中的端点：/order/{orderId}  <font color='red'>“{}”是占位符，自然表示orderId表示的是个变量</font>
...
<font color='red'>当前资源是 /read，关联资源是 /write，统计的是关联资源的访问量，假如 /write触发了阈值，就对 /read限流。从这个场景看，/write的优先级比 /read的优先级高</font>



**使用场景**：比如用户支付时需要修改订单状态，同时用户要查询订单。查询和修改操作会争抢数据库锁，产生竞争。业务需求是优先支付和更新订单的业务，因此当修改订单业务触发阈值时，需要对查询订单业务限流。

<font color='red'>当前资源是查询，关联资源是修改订单。统计的是修改订单的阈值，假如修改订单的阈值触发了，那就对查询业务限流。虽然很奇怪，但是这是关联模式的应用</font>

<font color='cornflowerblue'>简而言之，如果用关联模式，优先级高和优先级低的资源是有竞争关系的，当前资源是优先级低的，关联资源是优先级高的，统计的是优先级高的访问次数，到达阈值之后限流的是优先级低的阈值</font>
...
<font color='red'>如果只希望统计从/test2进入到/common的请求，则可以这样配置</font>：
...
2. 在OrderController中，改造/order/query端点，调用OrderService中的queryGoods方法  <font color='red'>从/order/query访问queryGoods</font>

3. 在OrderController中添加一个/order/save的端点，调用OrderService的queryGoods方法 <font color='red'>从/order/save访问queryGoods</font>

4. 给queryGoods设置限流规则，从/order/query进入queryGoods的方法限制QPS必须小于2 <font color='red'>给queryGoods设置限流规则，使得/order/query到queryGoods的访问受限</font>
...
默认情况下，<font color='red'>OrderService中的方法是不被Sentinel监控的，需要我们自己通过注解来标记要监控的方法。</font>

给OrderService的queryGoods方法添加<font color='red'>@SentinelResource</font>注解：
...
链路模式中，是对不同来源的两个链路做监控。<font color='red'>但是sentinel默认会给进入SpringMVC的所有请求设置同一个root资源，会导致链路模式失效。</font> <font color='red'>因为链路模式强调的是不同源访问同一个资源，比如 `/order/query` 和 `/order/save` 访问 `goods` 这个资源，那假如都同源了，那还链路模式个几把</font>
...
•<font color='red'>直接：对当前资源限流</font>

•<font color='red'>关联：高优先级资源触发阈值，对低优先级资源限流。</font> <font color='cornflowerblue'>重点是俩url竞争，对低优先级的做限流 </font>

•<font color='red'>链路：阈值统计时，只统计从指定资源进入当前资源的请求，是对请求来源的限流</font>  <font color='cornflowerblue'>重点是对请求来源做限流</font>
...
warm up也叫**预热模式**，是应对服务冷启动的一种方案。请求阈值初始值是 maxThreshold / coldFactor，<font color='red'>即三分之一</font>持续指定时长后，逐渐提高到maxThreshold值。而coldFactor的默认值是3.
...
<font color='red'>当请求超过QPS阈值时，快速失败和warm up 会拒绝新的请求并抛出异常。warm up的阈值初始是最大的三分之一，然后会慢慢升到最高，整个过程中，超过阈值的请求会被拒绝然后抛出异常</font>



### 2.3.2.排队等待

<font color='red'>当请求超过QPS阈值时，快速失败和warm up 会拒绝新的请求并抛出异常。</font>
...
<font color='red'>QPS肯定是平滑的，因为我们设置的接收的QPS最大是10，超出了最大的QPS之后来新的请求如果等待时间没有超过等待时间，那就放入队列，所以队列满了之后就会有部分请求失败</font>
...
<font color='red'>流控效果有哪些？</font>

- <font color='red'>快速失败：QPS超过阈值时，拒绝新的请求</font>

- <font color='red'>warm up： QPS超过阈值时，拒绝新的请求；QPS阈值是逐渐提升的，可以避免冷启动时高并发导致服务宕机。</font>

- <font color='red'>排队等待：请求会进入队列，按照阈值允许的时间间隔依次执行请求；如果请求预期等待时长大于超时时间，直接拒绝</font>
...
<font color='red'>简而言之就是对资源的不同参数进行限流，这就是热点参数限流</font>



# 3.隔离和降级

<font color='red'>限流是一种预防措施，虽然限流可以尽量避免因高并发而引起的服务故障，但服务还会因为其它原因而故障。</font>

而要将这些故障<font color='red'>控制在一定范围，避免雪崩</font>，就要靠**线程隔离**（舱壁模式）和**熔断降级**手段了。
...
**熔断降级**：是在调用方这边加入断路器，<font color='red'>统计对服务提供者的调用，如果调用的失败比例过高，则熔断该业务，不允许访问该服务的提供者了</font>。
...
可以看到，不管是线程隔离还是熔断降级，都是对**客户端**（调用方）的保护。<font color='red'>需要在**调用方** 发起远程调用时做线程隔离、或者服务熔断</font>。

而我们的微服务远程调用都是基于Feign来完成的，<font color='red'>因此我们需要将Feign与Sentinel整合，在Feign里面实现线程隔离和服务熔断</font>。



## 3.1.FeignClient整合Sentinel

<font color='red'>SpringCloud中，微服务调用都是通过Feign来实现的，因此做客户端保护必须整合Feign和Sentinel。</font>
...
<font color='red'>业务失败后，不能直接报错，而应该返回用户一个友好提示或者默认结果，这个就是失败降级逻辑。</font>

给FeignClient编写失败后的降级逻辑

①方式一：FallbackClass，无法对远程调用的异常做处理

②方式二：<font color='red'>FallbackFactory，可以对远程调用的异常做处理，我们选择这种</font>

<font color='cornflowerblue'>注意这里说的是降级处理，也就是说请求被拒绝的时候（http请求被拒绝的时候），feign返回的是降级的处理结果，包括 `线程隔离` 和 `熔断降级` 的返回结果</font>
...
@Slf4j
// 实现FallbackFactory接口就可以创建远程调用的降级策略
...
<font color='cornflowerblue'>感觉这么一配置，请求被拒绝的时候就会返回降级策略</font>
...
<font color='red'>Feign整合Sentinel的步骤：</font>
...
**线程池隔离**：<font color='red'>给每个服务调用业务分配一个线程池，利用线程池本身实现隔离效果</font>

**信号量隔离**：<font color='red'>不创建线程池，而是计数器模式，记录业务使用的线程数量，达到信号量上限时，禁止新的请求。</font>

<font color='red'>信号量隔离的做法很聪明真的，记录线程的数量，就不会像线程池隔离那样会有浪费，假如服务A不能用，那么分配给业务A的10个线程还是只能给A用，但是信号量就不一样了，我记录你最多能用几个，你要是当前不能用，那就不会占用线程，自然就不会有浪费</font>
...
- 线程数：<font color='red'>是该资源能使用用的tomcat线程数的最大值。也就是通过限制线程数量，实现**线程隔离**（舱壁模式）。</font>
...
解读：<font color='red'>RT超过500ms的调用是慢调用，统计最近10000ms内的请求，如果请求量超过10次，并且慢调用比例不低于0.5，则触发熔断，熔断时长为5秒。然后进入half-open状态，放行一次请求做测试。</font>
...
<font color='red'>触发了熔断，请求时长缩短至5ms，快速失败了，并且走降级逻辑，返回的null</font>
...
<font color='red'>你想问我们微服务不都是从网关来的吗？不一定，如果公司有内鬼把服务暴露出去了，那么此时服务肯定是需要对来源做一个判断的</font>
...
<font color='red'>这个方法的作用就是从request对象中，获取请求者的origin值（请求头参数）并返回。</font>
...
<font color='red'>貌似一个服务写一个就够了？每个服务定义一个RequestOriginParser的实现类，sentinel是通过调用这个类的parseOrigin方法来获得访问这个服务的origin的，通过判断请求的origin来去白名单或者黑名单来控制请求是否被接收的</font>
...
<font color='red'>push模式为推荐模式</font>
...
<font color='red'>阿里很鸡贼，阿里把持久化弄成了收费的，如果需要自己搞sentinel配置的持久化，就需要自己改sentinel的源码</font>
