...
<font color='red'>使用AVL的话一层只有两个节点，而B+树肯定是要查询到叶子节点的，所以会导致读取太多的页，从而IO太多次</font>
...
<font color='red'>B树本质上是多路平衡查找树，即多叉版本的AVL，而B+树不仅是多路平衡的，而且索引页只做索引，最终所有的数据都要放在叶子节点里面</font>
...
- 如果B+树有2层，最多能存放 `1600×16=25600` 条记录。
  ...
  <font color='red'>一共两层，第一层是目录页，第二层是数据页，一个页16kb=16000字节，一共可以放1600个指针，即指向1600个数据页，每个数据页放16条数据，2层一共放1600*16条数据</font>

- 如果B+树有3层，最多能存放 `1600×1600×16=40960000` 条记录。

  <font color='red'>明白第二层怎么算的，再多的层数只需要乘以一个目录页可以放多少个目录了，这里一个目录页存放1600个指针。</font>
...
- 自增主键能够维持底层数据顺序写入，<font color='red'>可以防止页分裂</font>
- 读取可以由b+树的二分查找定位
- 支持范围查找，范围数据自带顺序
...
- 聚簇索引插入新值比采用非聚簇索引插入新值的速度要慢很多，因为插入要保证主键不能重复
  ...
  <font color='red'>更新聚簇索引一定是比更新非聚簇索引慢的，因为更新聚簇索引不仅要更改聚簇索引所在的B+树，而且还要修改所有非聚簇索引的树，而非聚簇索引的修改只需要修改对应的那个B+树就好来了</font>
...
<font color='red'>这里肯定说的是innodb，因为myisam的所有的索引结构都存放的都是数据的地址</font>
...
age -> index（这里age是二级索引）

select * from user where age >20 ;

第一次 取回id，第二次（回表）根据id拿到完整数据

age,name -> index（这里是age和name构成的联合二级索引）

select age from user where age >20 and name like"张%" ;

覆盖索引不会回表查询，查询效率也是比较高的

<font color='red'>在非聚簇索引查询的时候，需要查询的内容在不需要回表的情况下就完成了查询，比如select里就查id和age，id是主键，age是非主键，那么从age对应的那个B+树里查询到叶子节点里，肯定既有age又有id，所以就不需要回表，这就是索引覆盖</font>

<font color='cornflowerblue'>简单来说，索引覆盖可以简单看成不需要回表</font>
...
<font color='red'>发生索引覆盖就不需要回表</font>
...
<font color='red'>这里影响的是插入的性能，因为是唯一性索引，所以会在插入前花费一点时间看一下会不会重复，普通索引就不会出现这个问题，因为普通索引没有唯一性要求</font>
...
<font color='red'>即忽略唯一索引对insert的影响，这远远不如唯一性索引查询时带来的速度提升，利大于弊</font>
...
- 参与mysql函数计算的列不适合建索引<font color='red'>函数计算会导致索引失效，因为mysql不知道一个数据经过函数计算后是什么B样，自然使用不了索引</font>
...
- <font color='red'>简单来说没有索引下推就是找到一条符合二级索引过滤条件的一条记录直接回表，然后把回表得到的数据返回给server，在server判断其他条件是否成立来决定这条记录是否可以构成结果集，这就存在一种情况假如所有数据都是经过回表后才发现不符合所有的where里的条件，这回表就相当于是白回表了</font>

  <font color='red'>而有索引下推的就是先把能进行判断的条件先判断完了，再去回表，很明显优于没有索引下推</font>

- <font color='cornflowerblue'>注意没有索引下推的情况是存储引擎先判断第一个条件，然后回表，然后把数据发送给server层，然后server去判断是否符合剩下的条件</font>
...
<font color='red'>这个很好理解，我们并不知道数据经过函数计算之后得到的是个什么B玩意，也就是说所有数据都有可能经过函数计算变成符合sql逻辑的数据，自然索引会失效</font>
...
<font color='red'>通配符直接把所有数据都匹配到了，自然索引失效</font>
...
<font color='red'>等值的话索引肯定生效，去B+树里肯定好找</font> 

<font color='red'>不等值匹配我是这么理解的，B+树的数据都在那里，而且本质上B+树就是升级的多路平衡查找树，既然是查找那么一定是去找某个东西，那么索引生效的情况一定是你去找某个东西，这样就能达到对数级别的时间复杂度，但是你现在的目标是不等于某个东西，这个目标在B+树里没法用，自然索引失效</font>
...
<font color='red'>关于等于和不等于 与 is null和is not null我认为两种对立索引失效和生效的原因不同，对于等值和不等值匹配的索引生效和失效就看上面就好了，而is null和is not null需要有新的视角去看待</font>

<font color='cornflowerblue'>is null 和 is not null如果出现在sql里，那么一定是出现在二级索引的使用上，因为聚簇索引完全不需要考虑是否为null的情况，那么既然确立了is null 和 is not null是发生在二级索引的情况下，我们再来考虑，一般一个字段里，正常情况下肯定是null的少，is not null的多，那么这时候用is not null去过滤条件，回表的次数肯定会特别多，那么此时效率就不如全表扫描了，因为回表次数太多了。那么非正常情况下，一个字段为null的多，is not null的少，假如此时用is not null去过滤，发生回表的次数会比较低，此时使用is not null时，索引就不会失效，而且type是range，但是此时用is null去过滤，回表次数就会多，反而此时用is null过滤就会索引失效，以后面试就这么说吧</font>

<font color='red'>简而言之，is null和is not null作为过滤条件时，索引的失效和生效主要看执行器的选择</font>
...
- 类型转换导致索引失效<font color='red'>原因同函数导致索引失效</font>
...
- 复合索引未用左列字段失效 <font color='red'>这个不用解释</font>
...
<font color='red'>注意all是全表扫描</font>

<font color='red'>这里因为select后面的是 * ，所以肯定是要回表的了，自然是全表扫描了</font>

select name,id  from user where  name like '%明'

type=index

<font color='red'>index是发生索引覆盖，但是扫描了全部的索引记录</font>

<font color='red'>这里只需要把name对应的B+树的全部叶子节点里面的记录都找一遍就好了（因为发生了索引覆盖）</font>

<font color='red'>可以发现这种情况下，虽然%在前面，但是索引并没有完全失效，只不过是没有高效的使用索引，而是把全部的所有索引记录扫描了</font>
...
<font color='red'>相当于是看一下优化路径，第二行的语句执行的结果是以json的形式展示出来所有计划的成本，以及最终选择了什么计划，以及最终计划的花费之类</font>
...
<font color='red'>假如联合索引（A，B，C），where的过滤条件是 A=1 and B>20 and C=3，以为B+树是按照A，B，C的顺序建立的，一旦执行了B>20，那么C其实就不好再利用B+树了，因为A=1好找，对于A过滤完然后找到大于20的B，此时C要找等于3的，其实是不好找的，即C那一段的联合索引就没有生效，但是假如联合索引是（A，C，B），过滤条件不变，我们对于A过滤完，然后C过滤完，剩下对于B直接找20，然后对应的那一条记录右面的叶子节点全是想要的值，可以发现此时联合索引都生效了，这是把范围查询的索引放在联合索引的位置中的后面位置的原因</font>
...
没有过滤条件不走索引<font color='red'>即没有where和limit</font>
...
<font color='red'>假如忽视order by，那么上面的sql就是没有过滤的查询，你都没有过滤了，那查询的就是全表了（type自然也是all），都全表查询了还要啥索引</font>
...
- sort_buffer可供排序的内存缓冲区大小<font color='red'>，即在内存排序的时候就在这块buffer里面排序，这个参数决定在排序的时候buffer是否够用</font>
- max_length_for_sort_data 单行所有字段总和限制，超过这个大小启动双路排序<font color='red'>这个参数决定当没有发生索引覆盖的时候，是使用单路排序和双路排序的临界点</font>
...
2. 判断索引内容是否覆盖select的字段
   1. 如果覆盖索引，select的字段和排序都在索引上，那么在内存中进行排序，排序后输出结果
      ...
      <font color='red'>即只要发生索引覆盖，就在内存里排序，就是单路排序</font>
   
   2. 如果索引没有覆盖查询字段，接下来计算select的字段是否超过max_length_for_sort_data限制，如果超过，启动双路排序，否则使用单路
   
      <font color='red'>如果没有发生索引覆盖，如果select的单行的字段没超过max_length_for_sort_data就是单路排序，否则就是双路排序</font>
...
单路排序：一次取出所有字段进行排序，内存不够用的时候会使用磁盘<font color='red'> ，即直接在内存排序</font>

双路排序：取出排序字段进行排序，排序完成后再次回表查询所需要的其他字段。<font color='red'>即要把排序的字段进行排序，然后回表</font>
...
<font color='red'>双路排序就是从磁盘加载到内存一部分，然后在内存排序之后，然后再回表（再从磁盘里加载），整个过程回表两次</font>

**单路排序（快）**

从磁盘读取查询需要的所有列，按照order by列在buffer对它们进行排序，然后扫描排序后的列表进行输出， 它的效率更快一些，避免了第二次读取数据。并且把随机IO变成了顺序IO，但是它会使用更多的空间， 因为它把每一行都保存在内存中了。

<font color='red'>单路排序就是纯纯把数据加载到sort_buffer里面去排序，除此之外没别的</font>
...
Ø 增大sort_buffer_size参数的设置 <font color='red'>增大排序占用的buffer大小</font>

Ø 增大max_length_for_sort_data参数的设置  <font color='red'>这个增大会减少触发 </font>
...
<font color='red'>select * 的时候，假如用单路排序，会加载所有的列到内存中，* 会导致sort_buffer不够用，导致使用磁盘的空间，会增大IO，假如使用双路排序，</font>
...
<font color='red'>提高这个参数会减少双路排序的次数</font>
...
- group by没有过滤条件，也可以用上索引。Order By 必须有过滤条件才能使用上索引。
  ...
  <font color='red'>因为group by需要根据字段找，所以会用到索引，order by如果没有过滤条件相当于全表扫描，自然不用索引</font>
...
<font color='red'>主要还是看表里的数据 is null 多还是 is not null 多，从而决定是否使用索引</font>
...
7. <font color='red'>相同的sql使用函数产生的效果可能也不同，比如运行同一个NOW() 函数，得到的结果肯定不一样，那样就算sql长的一样，得到的结果肯定也不一样</font>
...
- TCP 连接收到请求后，必须要分配给一个线程专门与这个客户端的交互。所以还会有个线程池，去走后面的流程。每一个连接从线程池中获取线程，省去了创建和销毁线程的开销。
  ...
  <font color='red'>和网络编程不同，数据库一定会给连接过来的客户端开一个线程去完成数据库与客户端的交互</font>
...
<font color='red'>简单来说，server层也就这几个部分</font>

1. <font color='red'>sql接口：接收sql，返回结果</font>
2. <font color='red'>解析器：把sql解析成语法树</font>
3. <font color='red'>优化器：选择一个好的执行计划</font>
4. <font color='red'>缓存组件：记录一些缓存，还有一些buffer</font>
...
**接下来，**`MySQL解析器通过关键字将SQL语句进行解析，并生成一棵对应的解析树，`解析器使用MySQL语法规则验证和解析SQL语句。例如，它将验证是否使用了错误的关键字，或者使用关键字的顺序是否正确，引号能否前后匹配等；`预处理器则根据MySQL规则进一步检查解析树是否合法，`例如，这里将检查数据表和数据列是否存在，还会解析名字和别名，看是否有歧义等。`然后预处理器会进行查询重写，生成一棵新解析树。` <font color='red'>即先语法分析去判断sql是否符合语法规定，预处理器再进行语义分析判断你用到的表名、列名是否正确</font>

**接下来，**`查询优化器将解析树转化成执行计划。`MySQL优化程序会对我们的语句做一些优化，如子查询转换为连接、表达式简化等等。优化的结果就是生成一个执行计划，这个执行计划表明了应该使用哪些索引执行查询，以及表之间的连接顺序是啥样，等等。我们可以使用EXPLAIN语句来查看某个语句的执行计划。<font color='red'>简单来说就是对语句进行一个优化，比如子查询优化为表连接，逻辑简化之类，然后生成一个执行计划，计划里有用哪些索引，表的连接顺序等等，可以用explain看sql的执行计划</font>

**最后，**`进入执行器阶段。`完成查询优化后，`查询执行引擎`会按照生成的执行计划调用存储引擎提供的接口执行SQL查询并将结果返回给客户端。在MySQL8以下的版本，如果设置了查询缓存，这时会将查询结果进行缓存，再返回给客户端。<font color='red'>简单来说，就是拿刚才的执行计划去调用执行引擎提供的接口，然后得到结果返回给客户端</font>
...
​	<font color='red'>Change Buffer是Buffer Pool里面的内容，用来放脏页</font>

​	<font color='red'>Log Buffer 放的是redo log等生成的日志记录</font>

**Log Buffer**：用来缓存redolog

**Adaptive Hash Index**：自适应哈希索引 <font color='red'>用来快速找到某个页</font>
...
<font color='red'>因为MyISAM不支持事务，所以并发的插和读是有风险的，所以当业务只有几乎读，就用MyISAM，否则Innodb</font>
...
<font color='red'>从一个合法状态到另一个合法状态</font>
...
<font color='red'>幻读一定是由插入导致的</font>
...
<font color='red'>配置事务隔离级别解决隔离性的问题</font>
...
<font color='red'>隔离性的实现无非就是靠锁和MVCC了，锁无非就是读写锁咯</font>
...
- 行锁（记录锁）：解决的就是**多个事务同时更新一行数据**，<font color='red'>行锁不用解释，肯定解决的是多个事务同时更新数据的问题</font>
- 间隙锁：解决的就是**多个事务同时更新多行数据**

下列操作属于锁定读

```
select ... lock in share mode（上共享锁的读）
select ... for update （上排他锁的读）
insert、update、delete （增删改默认都是排他锁）
```

<font color='red'>锁只有共享锁和排他锁，在共享锁与排他锁的组合中，只有共享锁和共享锁的组合是不会阻塞的，剩下都会互斥，如共享锁和排他锁，排他锁和共享锁，排他锁和排他锁的组合情况都是发生阻塞的</font>
...
<font color='red'>就是常规的select</font>
...
<font color='red'>MVCC最主要解决的问题就是非锁定读</font>
...
<font color='red'>MVCC 的实现依赖于：隐藏字段、Read View、undo log</font>
...
- A 7-byte `DB_ROLL_PTR` 回滚指针，指向该行的 undo log 。如果该行未被更新，则为空. <font color='red'>也就是这个字段以近似于链表的形式形成一条版本链，越往下版本越老</font>
- A 6-byte `DB_ROW_ID` 如果没有设置主键且该表没有唯一非空索引时，`InnoDB` 会使用该 id 来生成聚簇索引.

<font color='red'>三个隐藏字段，事务id，row_id，回滚指针</font>
...
<font color='red'>在事务对行记录做了修改后，行记录的隐藏字段trx_id都会记录这个事务的id并且生成对应的版本，所以在不同事物要对这个行记录进行读取的时候就可以产生一个可见版本，而Read View就是干这个事情的，用来提供给当前某个事务操作某条行记录时这个行记录对于这个事务的可见的版本，至于说怎么判断是否可见，这个是一个新的问题</font>
...
<font color='red'>也就是说undo log不仅可以用来回滚数据，还可以给MVCC提供功能，可以读出来可见的版本，以此实现不上锁的读</font>
...
<font color='red'>关于任意两个意向锁之间不会产生冲突，这个很好理解，假如一个表有100行，第3行上了写锁，那么这个表就有意向写锁了，这时候再给第5行上锁，也会给表再上意向锁，这是很合理的，毕竟第三行和第五行的上锁是随便上的，别忘了意向锁的作用</font>

<font color='cornflowerblue'>意向锁的好处就是想给上表锁的时候，就不用去查看每条记录是否上有锁，已经是否会发生锁互斥的问题了，通过查看意向锁，表明当前表内有记录正在上锁可以快速得到结果</font>
...
当前读 ：在锁定读（使用锁隔离事物）的时候读到的是最新版本的数据 <font color='red'>上锁了，读到的肯定是最新的数据</font>

快照读：可重复读（repeatable-read）下 mvcc生效读取的是数据的快照，并不是最新版本的数据（未提交事物的数据） <font color='red'>本质上读取到的是当前ReadView里对于当前事务可见的那个行记录的版本</font>
...
<font color='red'>select for update 就会给遍历到的数据都上排他锁，其他事务想上锁的话暂时是不行的，会互斥</font>
...
<font color='red'>bin log是逻辑日志</font>
...
<font color='red'>redo log是物理日志，某个页修改了xxx</font>
...
<font color='red'>undo log 也是逻辑日志</font>
...
<font color='red'>binlog 写入策略：</font>

**1、sync_binlog=0** 的时候，表示每次提交事务binlog不会马上写入到磁盘，而是先写到page cache,相对于磁盘写入来说写page cache要快得多,不过在Mysql 崩溃的时候会有丢失日志的风险。<font color='red'>事务提交了之后，先写入到页缓存里（write过程），然后由后台线程去执行刷盘（fsync过程）</font>

**2、sync_binlog=1** 的时候，表示每次提交事务都会执行 fsync 写入到磁盘 ；<font color='red'>每次事务提交直接刷盘</font>

**3、sync_binlog的值大于1** 的时候，表示每次提交事务都 先写到page cach，只有等到积累了N个事务之后才fsync 写入到磁盘，同样在此设置下Mysql 崩溃的时候会有丢失N个事务日志的风险。<font color='red'>事务提交之后，还是先写入到页缓存，但是是积累了N个事务才写入磁盘</font>



<font color='cornflowerblue'>对于bin log，从binlog cache写入页缓存叫write过程，从页缓存写入磁盘叫fsync过程</font>



很显然三种模式下，sync_binlog=1 是强一致的选择，选择0或者N的情况下在极端情况下就会有丢失日志的风险，具体选择什么模式还是得看系统对于一致性的要求。

<font color='red'>bin log日志主要看业务需求</font>
...
<font color='red'>redo log的刷盘策略</font>
...
<font color='red'>0：每秒从log buffer里刷盘</font>

<font color='red'>1：事务提交就刷盘</font>

<font color='red'>2：每次事务提交从log buffer写入页缓存，由os决定什么时候写入磁盘</font>



<font color='cornflowerblue'>其实记录参数不重要，只需要知道参数为1的时候就是提交事务去刷盘，其他的参数都和页缓存有关系，都提高了性能，但是事务记录丢失的风险</font>
...
- **prepare**：redolog写入log buffer，并fsync持久化到磁盘，在redolog事务中记录2PC的XID，在redolog事务打上prepare标识
  ...
  <font color='red'>redo log 刷盘完了给redo log事务打prepare标记</font>

- **commit**：binlog写入log buffer，并fsync持久化到磁盘，在binlog事务中记录2PC的XID，同时在redolog事务打上commit标识

  <font color='red'>binlog 刷盘完了给redo log事务打commit标记</font>

<font color='cornflowerblue'>只有给redo log事务打了commit标记，我们才认为redo log做成功了</font>

![img](v2-a48d01fd3478ba4d68207fc7ce757658_r.jpg)

<font color='red'>上面那个图有必要解释一下，Session发来了Commit，server接受到了之后，会先去让Engine，即存储引擎，去刷盘redo log（这里也可以看出redo log的刷盘是由Engine去做的），等存储引擎刷盘完redo log之后，返回给server一个ok。从server接受到commit让engine去刷盘redo log然后返回ok，这个阶段是prepare的阶段，即第一阶段。</font>

<font color='red'>第一阶段完成之后，开始刷盘 bin log，bin log的过程包括write和fsync，等bin log刷盘完之后，server会让engine给redo log事务打上commit标记，此时，redo log才算真正的提交完成，这时候server再返回session一个ok，表示session提交的commit才算完成</font>



<font color='cornflowerblue'>redo log 和 binlog 的两阶段提交目的就是让 redo log 和 binlog 记录的变化是一模一样的，redo log刷盘完了显示prepare，等binlog刷盘完了，redo log才成了commit，此时事务的提交才算完成。这样真正保证了redo log和binlog的变化是统一的，不然可能存在不统一的变化</font>
...
- binlog_format=STATEMENT（默认）：数据操作的时间，同步时不一致 每一条会修改数据的sql语句会记录到binlog中。优点是并不需要记录每一 条sql语句和每一行的 数据变化，减少了binlog日志量，节约IO，提高性能。缺点是在某些情况下会导致 master-slave 中的数据不一致( 如sleep()函数， last_insert_id()，以及user-defined functions(udf)等会 出	现 问题)
  ...
  <font color='red'>记录sql逻辑，不记录数据变化，但是会有数据不一致问题，比如now()函数的结果不同</font>
...
- binlog_format=ROW：批量数据操作时，效率低   不记录每条sql语句的上下文信息，仅需记录哪条数据被修改了，修改成什么样 了。而且不会出 现某些特定情况下的存储过程、或function、或trigger的调用和触发无法被正确复制的 问题。缺 点是会产生大量的日志，尤其是alter table的时候会让日志暴涨。
  ...
  <font color='red'>记录sql变化的数据，记录变化数据的值，但仍是逻辑改变，不是redo log的物理改变</font>
