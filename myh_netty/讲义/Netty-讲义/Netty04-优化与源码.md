...
* 本质序列化和反序列化都需要的是 byte[]

目前的代码仅支持 Java 自带的序列化，反序列化机制，核心代码如下

```java
// jdk的序列化和反序列化的写法算是固定的了
// 反序列化
@Override
public <T> T deserialize(Class<T> clazz, byte[] bytes) {
  try {
		// 字节数组输入流接受一个byte[]，对象输入流接受一个字节数组输入流
...
enum SerializerAlgorithm implements Serializer {
  ...
  // 和上面相同
    ...
    Json {
        @Override
        public <T> T deserialize(Class<T> clazz, byte[] bytes) {
          	// byte[]自定字符集->字符串(json格式)->对象(指定对象类型)
            return new Gson().fromJson(new String(bytes, StandardCharsets.UTF_8), clazz);
        }

        @Override
        public <T> byte[] serialize(T object) {
          	// 对象->字符串(json格式)->指定字符集变成byte[]
            return new Gson().toJson(object).getBytes(StandardCharsets.UTF_8);
        }
    };

    // 需要从协议的字节中得到是哪种序列化算法
  	// 在自定义协议里面有个字段记录了使用了哪种序列化算法(以索引方式，这个索引方式和枚举里面的索引相同)
...
public class MessageCodecSharable extends MessageToMessageCodec<ByteBuf, Message> {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
        ...
        // 确定具体消息类型
      	// 因为要知道反序列化生成的是什么对象，所以需要传入Class对象，Class只需要从对应的map获得就行
...
public abstract class Message implements Serializable {
    ...
    public static final int PongMessage = 15;
  	// 自定义协议里面放的都是byte数字，表示当前这条消息是哪种类型
...
**其实就是设置了一个定时任务，第8行，如果定时任务超过设置的时间，就会报错**
...
要注意的是 accept queue 里存放的是当前因为正在有连接进行 accept 而无法 accept 的连接信息，也就是说新的tcp已经建立好了，但是服务器正在 accept 之前建立好的请求，所以你当前这个请求先等等，去 accept queue 里面等等， 所以accept queue 里面的连接信息都是可以被 accept 的，accept方法不断的从 accept queue 里面拿到连接信息去进行accpet。

一般 accept queue 是不会满的，因为accept其实也是很快的，相当于不断从 accept queue 里面拿连接信息，导致 accept queue 不是很容易满，一般都是突然间的高并发让 accept 方法不断运行的过程中， accept queue 还是满了的时候，才会有异常。
...
* accept queue - 全连接队列
  ...
  * 如果 accpet queue 队列满了，server 将发送一个拒绝连接的错误信息到 client，**即客户端收到一个拒绝连接的错误信息**
...
第1个客户端正常被服务端accept，第2,3个客户端与服务端都建立tcp了，但是因为服务端没有调用accept，所以这俩放在 accept queue 里面，到第4个客户端连接的时候，因为 accept queue 满了，所以客户端收到连接异常
...
* 属于操作系统参数，设置当前线程最大能拥有的文件操作符的个数，即目的是为了不让这个线程打开过多的文件



#### 4）TCP_NODELAY

* 属于 SocketChannal 参数，小的数据包容易不发，即如果设置延迟，那么小的数据包凑齐大的数据包之后才会一起发出去，如果没有设置延迟，那就所有小数据包都会发出去，默认是没有延迟
...
* 这个其实就是滑动窗口的缓冲区的设置，分为发送方缓冲区和接收方缓冲区，一般不用自己设置，都有操作系统根据网络流量来自动设置大小
...
入口 `io.netty.bootstrap.ServerBootstrap#bind2`
